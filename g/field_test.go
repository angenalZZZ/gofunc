...
func TestField_NotSettable(t *testing.T) {
	...
	if err := s.Field("A").Set("newValue"); err != g.ErrNotSettable {
		t.Errorf("Trying to set non-settable field should error with %q. Got %q instead.", g.ErrNotSettable, err)
	}
}
