...
func ExampleMap_tags() {
	...
	type Server struct {
		Name    string `struct:"server_name"`
		ID      int32  `struct:"server_id"`
		Enabled bool   `struct:"enabled"`
	}
...
func ExampleMap_omitNested() {
	...
	type Server struct {
		Name string    `struct:"server_name"`
		ID   int32     `struct:"server_id"`
		Time time.Time `struct:"time,omitnested"` // do not convert to map[string]interface{}
	}
...
func ExampleMap_omitEmpty() {
	...
	type Server struct {
		Name     string `struct:",omitempty"`
		ID       int32  `struct:"server_id,omitempty"`
		Location string
	}
...
func ExampleValues_tags() {
	...
	type Server struct {
		...
		ID       int32
		Enabled  bool
		Location Location `struct:"-"` // values from location are not included anymore
	}
...
func ExampleHasZero() {
	...
	// going to be checked because we added the "struct" tag to the field.
	type Access struct {
		Name         string
		LastAccessed time.Time
		Number       int
