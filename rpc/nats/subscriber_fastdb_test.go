...
func TestSubscriberFastDb(t *testing.T) {
	// New Client Connect.
	nat.Subject = "TestSubscriberFastDb"
	...
	sub := nat.NewSubscriberFastDb(nat.Conn, nat.Subject, data.RootDir)
...
// Take time 16.4s:60bytes, 17.6s:500bytes to process 1 million records,
// run times 60905:60Bytes, 56780:500bytes Qps.(4CPU+16G+MHD)
func TestBenchSubscriberFastDb(t *testing.T) {
	// New Client Connect.
	nat.Subject = "BenchmarkSubscriberFastDb"
	nat.Conn, err = nat.New("nats.go", "", "", "HGJ766GR767FKJU0", "", "")
	if err != nil {
		t.Fatalf("[nats] failed to connect: %s\n", err.Error())
	}
