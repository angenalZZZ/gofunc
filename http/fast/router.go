...
func (app *Fast) nextRoute(ctx *Ctx) {
...
func (app *Fast) handler(fctx *fasthttp.RequestCtx) {
...
func (app *Fast) registerMethod(method, path string, handlers ...func(*Ctx)) {
...
func (app *Fast) registerStatic(prefix, root string, config ...Static) {
	...
	app.routes = append(app.routes, &Route{
		...
		Handler: func(c *Ctx) {
			...
			if c.method == "GET" || c.method == "HEAD" {
				// Do stuff
				if wildcard {
					c.C.Request.SetRequestURI(prefix)
				}
				// Serve file
				fileHandler(c.C)
				// End response when file is found
				if c.C.Response.StatusCode() != 404 {
					return
				}
			}
			c.Next()
