...
func TestMD5(t *testing.T) {
	...
	encryptedStringGo := f.CryptoMD5(origData)
...
func TestCryptoAes(t *testing.T) {
	...
	encryptedBytes := f.CryptoAesCBCEncrypt(origData, key, iv)
	...
	origDataRaw := f.CryptoAesCBCDecrypt(encryptedRaw, key, iv)
	...
	salt := []byte("hgt!16kl")
	encrypted1 := f.CryptoAesCBCEncryptWithHmacSHA1(origData, key, salt, iv, 1000, 32)
	...
	origDataRaw1 := f.CryptoAesCBCDecryptWithHmacSHA1(encryptedRaw1, key, salt, iv, 1000, 32)
...
func TestCryptoDes(t *testing.T) {
	...
	key := []byte("GFpU7S3v")
	iv := []byte("jkE49230Tf093b42")
	encryptedBytes := f.CryptoDesCBCEncrypt(origData, key, iv)
	encryptedString := hex.EncodeToString(encryptedBytes)
	...
	origDataRaw := f.CryptoDesCBCDecrypt(encryptedRaw, key, iv)
	if string(origDataRaw) != string(origData) {
		t.Fatal("origDataRaw != origData")
	}
	...
	encrypted := f.CryptoDesECBTripleEncrypt(origData, key)
...
func TestCryptoRSA(t *testing.T) {
	...
	publicKeyEncrypt := f.NewRSAPublicKey(publicKeyPemBytes)
	...
	privateKeyDecrypt := f.NewRSAPrivateKey(privateKeyPemBytes)
...
func TestCryptoAesCBCEncryptWithHmacSHA1_ABP(t *testing.T) {
	...
	encryptedBytes := f.CryptoAesCBCEncryptWithHmacSHA1(origData, key, salt, iv, 1000, 32)
