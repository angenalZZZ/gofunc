...
func (d *Death) WaitForDeath(closable ...io.Closer) (err error) {
	...
	count := len(closable)
	log.Println("Closing ", count, " objects")
	if count > 0 {
	...
	for {
		...
		case <-timer.C:
			...
			for _, c := range sentToClose {
				pks = append(pks, fmt.Sprintf("%s/%s", c.PKGPath, c.Name))
				log.Println("Failed to close: ", c.PKGPath, "/", c.Name)
			}
			return fmt.Errorf("%s", fmt.Sprintf("%s %s", s, strings.Join(pks, ", ")))
